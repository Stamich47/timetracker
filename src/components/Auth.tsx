...
const Auth: React.FC<AuthProps> = ({ onAuthSuccess }) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Show info message while redirecting
      setInfoMessage("Redirecting to Google for authentication...");
    } catch (err: unknown) {
      setError(
        err instanceof Error
          ? err.message
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-surface rounded-2xl shadow-xl p-8 border border-border">
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center px-4 py-3 border border-border rounded-lg text-secondary bg-surface hover:bg-surface-hover hover:shadow-md hover:border-border-hover focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 transform hover:scale-[1.02]"
          >
